...
import { History as HistoryIcon, Trash2, Trash, FileText, ListChecks, Users, AlertTriangle, Clock, FlaskConical } from "lucide-react";
...
export default function HistoryPage() {
  const { testCases, setTestCases, testPlans, setTestPlans, stories, setStories, scenarios, setScenarios, showToast } = useAppStore();
  const [activeTab, setActiveTab] = useState("all");
  const [showConfirmClear, setShowConfirmClear] = useState(false);

  // Normalise all items into a unified list with a type tag
  const allItems = [
    ...
    <div className="max-w-5xl mx-auto mt-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {[
          ...
          <div key={stat.label} className={`bg-card rounded-xl border ${stat.border} p-4 flex items-center gap-3`}>
            <div className={`text-3xl font-bold ${stat.color}`}>{stat.count}</div>
            <div className={`text-xs font-medium ${stat.color} opacity-80`}>{stat.label}</div>
          </div>
        ))}
      </div>
                ...
                <div className="flex items-center gap-4 min-w-0">
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${cfg.bg} ${cfg.border} ${cfg.color} whitespace-nowrap flex-shrink-0`}>
                    ...
                    <div className="min-w-0">
                      ...
                      {item._type === "testPlans" && item._sub && (
                        <p className="text-xs text-textMuted mt-0.5 truncate max-w-xl">{item._sub}</p>
                      )}
                      {item._type === "stories" && item.description && (
                        <p className="text-xs text-textMuted mt-0.5 truncate max-w-xl">{item.description}</p>
                      )}
                    </div>
                </div>
