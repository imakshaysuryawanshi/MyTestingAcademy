...
export default function Settings() {
  const { settings, setSettings, setStories, setTestPlans, setTestCases, setScenarios, showToast } = useAppStore();
    ...
    <div className="max-w-5xl mx-auto mt-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-card p-6 rounded-2xl border border-border shadow-sm hover:border-accent/40 transition">
          ...
          <div className="space-y-4">
            {["testPlan", "testCase", "coverage", "codeGen"].map(key => (
              <div key={key}>
                <label className="block text-sm font-medium text-gray-300 mb-1 capitalize">
                  {key.replace(/([A-Z])/g, " $1").trim()} Payload
                </label>
                <select
                ...
                >
                  {availablePrompts.length > 0
                    ? availablePrompts.map(p => <option key={p} value={p}>{p}.md</option>)
                    : <option value={localConfig.prompts[key]}>{localConfig.prompts[key]}.md</option>}
                </select>
              </div>
            ))}
          </div>
