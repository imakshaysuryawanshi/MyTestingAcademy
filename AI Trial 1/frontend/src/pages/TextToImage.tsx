...
import AspectSelector from '../components/parameters/AspectSelector';
...
export default function TextToImage() {
  ...
  const [model, setModel] = useState('Ollama (Local)');
  ...
  useEffect(() => {
    // Load history matching type on mount
    const allHistory = historyStore.getHistory();
    setHistory(allHistory.filter(h => h.type === 'image'));
  }, []);
  ...
  const handleGenerate = async () => {
    ...
    if (response.success) {
      ...
      setHistory(updatedHistory.filter(h => h.type === 'image'));
    }
